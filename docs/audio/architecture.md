...
- Compact and expanded player forms share stable logical edges for Minimize/Expand and Close, dock to the reading surface's bottom safe area, and expose a single session-progress value derived from the frozen queue, repetition, and media position.
- Expanded transport controls preserve 44px targets and flex within a 320px viewport instead of clipping Previous or Next beyond the screen. The expanded sheet contains horizontal overflow and bounds vertical scrolling to the available dynamic viewport.
