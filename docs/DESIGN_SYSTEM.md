...
- Quran Wird opens with an opaque overview before the Mushaf: one radial page-progress summary with a text equivalent, the last verified reading position, and **Continue reading** as its sole primary action. A page is recorded only by an explicit reader or overview action, never merely by loading it. Plans are one-at-a-time (30-day khatmah, daily pages, or custom duration) and use gentle redistribution language rather than streak failure.
