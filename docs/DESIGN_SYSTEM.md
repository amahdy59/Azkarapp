...
- Home keeps the five-prayer summary above its contextual hero row at every tier. The row uses a dominant two-thirds primary card and a one-third daily-evidence companion from the desktop component tier onward, and both cards stretch to the row height. The prayer hero contains the approved interactive post-prayer journey; the summary above it is stable prayer navigation and temporal status, not a second set of tracking controls. Today's Wird occupies its own full-width row so its three routine tiles respond to real container space instead of being compressed inside the companion column. The composition stays inside the dashboard measure and stacks in semantic order below the desktop tier.
