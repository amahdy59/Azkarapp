...
- The standalone 604-page Mushaf has no separate application header or footer. Surah selection, juz context, page number, bookmark, theme, difficult-word toggle, page recording, and Previous/Next controls live in reserved header/footer space inside the page and fade after reading begins without reflowing the reviewed 15 lines. A 44 px reveal control restores them; swipe and physical arrow keys remain equivalent inputs. The local reviewed Unicode page is always the immediate offline fallback. When available, the reader merges Quran Foundation `code_v2` and authoritative line numbers into that local semantic text, loads the official page-specific QCF V2 font, and uses the font's own verse-end glyphs. It caches recent remote page data/fonts at runtime but never makes network availability a prerequisite for reading. Difficult-word tint, underline, buttons, and reviewed meanings appear only while the explicit in-page toggle is on; the unmarked text remains the default.
