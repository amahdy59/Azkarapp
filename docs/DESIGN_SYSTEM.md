...
- **Each prayer has its own drawn sky**, in `PrayerSceneArt`: vector rather than photography, so there is no request to wait for, the hue holds in every theme, and the precache gains nothing. It is composed rather than drawn as one picture, because the hero has no fixed aspect ratio — the sky is a CSS gradient, the light is a fixed-size element placed at a percentage, and only the skyline is SVG, **fitted** to the card's width and anchored to its bottom edge. A band scaled to _cover_ shows a fragment of itself: at desktop width that made one dome the entire drawing. The light keeps to the trailing band, clear of the prayer and its time, and the hour is carried by how high it sits. Text over a scene uses fixed light-on-dark, as the Home hero does over its photograph.
...
- Home's contextual hero row uses a dominant two-thirds primary card and a one-third daily-evidence companion from the desktop component tier onward. Both cards stretch to the row height. The prayer hero already contains the approved interactive post-prayer journey; Home must not mount a second summary of that workflow. Today's Wird occupies its own full-width row so its three routine tiles respond to real container space instead of being compressed inside the companion column. The composition stays inside the dashboard measure and stacks in semantic order below the desktop tier.
