...
- The standalone 604-page Mushaf **is the whole screen** at every width: no card, no gutter, no letterbox, and no screen padding above or below it — the shell's safe-area insets are the only space outside the paper. On screens wider than the page's own proportion the paper still fills the viewport while the fifteen-line column keeps the Mushaf's measure, because width belongs to the chrome and not to line length. Its visible header has four labelled targets — Back, surah/juz index, the difficult-words switch, and Options — and the footer has four — Previous, Record page, Page number, and Next. Bookmark and theme use the shared Options menu as progressive disclosure; the difficult-word control is a `role="switch"` on the toolbar itself, not a menu entry, because it is the one reading aid a reader turns on and off mid-page. Both rows fade after reading begins without reflowing the reviewed 15 lines; a labelled 44 px reveal control restores them, and swipe plus physical arrow keys remain equivalent inputs. **The forward-pointing control advances the Mushaf in both directions**: the footer's end-side button and `ArrowRight` go to the next page, the start-side button and `ArrowLeft` go back (DEC-089). The reference is the King Fahd Complex Madani Mushaf in QCF v2, and both halves of it ship with the app — `public/data/mushaf` carries the reference line numbers and the `code_v2` glyph for every word, so a page turn makes no network request at all and the page font is served from Cache Storage after its first load. A page falls back to Unicode/Amiri only when its own glyphs or font are missing, and the fallback reproduces the same geometry rather than reflowing. Every page lays out fifteen equal line slots whether or not each carries words, each line is justified to both margins as in print, and a line whose natural width overruns the page is scaled down to fit — never clipped. A surah heading occupies exactly one line slot, as a thin ornamented band. Weight for legibility comes from a hairline text stroke, tuned per theme; QCF v2 is a single-weight face and synthetic bold smears its counters. Page turns cross-fade in 150 ms and honour reduced motion; dragging writes one transform and costs no React render. An eight-page in-memory cache plus a save-data-aware prefetch of both neighbours makes an ordinary turn a cache hit. Difficult-word tint, dotted underline, buttons, and reviewed meanings appear only while the switch is on; their inline metrics equal unmarked text so toggling cannot move a line.
