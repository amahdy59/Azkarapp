...
- The standalone 604-page Mushaf has no separate application header or footer. Its visible header has three labelled targets — Back, surah/juz index, and Options — while the footer has four labelled targets — Previous, Record page, Page number, and Next. Bookmark, theme, and the difficult-word switch use the shared Options menu as progressive disclosure. Both rows fade after reading begins without reflowing the reviewed 15 lines; a labelled 44 px reveal control restores them, and swipe plus physical arrow keys remain equivalent inputs. At the 320×700 reference viewport the page surface occupies at least 95% of the viewport with no horizontal overflow. QCF rendering is atomic: the reader resolves the reviewed local page, Quran Foundation `code_v2` data, and the matching page-specific QCF V2 font before mounting the page, so it never displays Unicode and then resizes into QCF. If the enhanced pair is unavailable after a bounded wait, it mounts the local Unicode/Amiri page once and remains fully readable offline. A four-page in-memory cache and save-data-aware adjacent-page preload make normal page turns immediate without unbounded memory or mandatory network access. Difficult-word tint, dotted underline, buttons, and reviewed meanings appear only while the explicit switch is on; their inline metrics must equal unmarked text so toggling cannot move a line.
