...
- The counter ring is 184 CSS px (76 CSS px for the compact pill it becomes when reading content would otherwise collide with it). This geometry is a fixed contract at every viewport and tier — see DEC-043 in `docs/agent/DECISION_LOG.md`, which restored it after a regression shrank it to 164px — and does not change with the wide-desktop reader below. Its numerals are plain `rem` values, so they already scale with the reader's textSize preference through the app-wide `--font-size` root variable (`src/app/theme.ts`'s `applyAppAppearance`, 14/16/18px for small/medium/large) like every other `rem`-sized element — no per-component textSize prop is needed or present, and adding one would double-scale on top of this. Both counter shapes share the same `AdaptiveCounterTrack` ring visual, and switching between them crossfades its content (reusing the `.zikr-step-enter` animation) instead of swapping instantly. For ordinary adhkar, a pointer activation anywhere on the Reader canvas counts, except interactive controls, menus, dialogs, editable fields, and scrollbars. Full surahs are counter-only: neither the Quran text nor canvas whitespace may change the count. The explicit reading and counter surfaces support Enter/Space and use `touch-action: manipulation` where applicable.
