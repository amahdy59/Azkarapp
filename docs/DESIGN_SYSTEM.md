...
- **The prayer card on Home is a door, and says so.** A chevron sits in its corner — not on the time's line, where it wrapped the longest time and dropped that card out of step with the other four — and its accessible name names the prayer it opens rather than the adhkar it used to. Its first tracking row reads **Prayed**, ticks for either recorded place, and names that place beneath it: rendering `mosque` alone meant a prayer recorded at home showed here as an empty box, which reads as unrecorded.
