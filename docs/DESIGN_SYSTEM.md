...
- The five-prayer summary is Home's stable schedule and navigation layer. Every prayer card contains its own calculated local time; the next prayer card also contains its countdown, with no duplicate device clock. It remains above the contextual row while that row changes between a prayer moment, the selected routine, and completion. The featured routine card stays focused on the selected zikr, its mode, and its progress. Once that selected time-of-day collection is complete, the completion card announces and enters surface/copy/check; it leaves in the reverse check/copy/surface order, then contracts its layout row so adjacent content moves without a jump. The routine CTA remains hidden for that collection until the recommendation changes.
- Each of the five post-prayer cards opens a prayer-specific reviewed flow. Shared adhkar appear for every prescribed prayer; Fajr and Maghrib additions are admitted only where their cited timing establishes them. Completion is stored under that prayer key and updates both Home and Progress without completing the other four prayers. Home uses the summary anatomy—identity, time, temporal state, and the route into the prayer—so it does not repeat fifteen recording controls before its continuation cards. The focused prayer and daily Progress views retain the full recording anatomy.
...
- **The prayer card on Home is a door, and says so.** A chevron sits beside its name—not on the time's line, where it wrapped the longest time and dropped that card out of step with the other four—and its accessible name names the prayer it opens rather than the adhkar it used to. Home stops at this summary. In the full Progress variant, the first tracking row reads **Prayed**, ticks for either recorded place, and names that place beneath it: rendering `mosque` alone meant a prayer recorded at home showed here as an empty box, which reads as unrecorded.
