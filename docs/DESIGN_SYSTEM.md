...
- **One action, one glyph.** Two controls that do different things may not share an icon, however related they sound. `BookOpen` is the zikr benefit and its reference sheet; word meanings — a glossary of the words on the page — uses `Translate`, and keeps it in both states rather than becoming a checkmark when it is on, which would be a third meaning for a control that has two.
...
- Before counting, the only visible instruction is localized and mode-specific: “Tap anywhere to count” for ordinary adhkar — “Click anywhere or press Space to count” from the tablet tier up — and “Tap counter when finished” inside the explicit counter for full surahs. Do not add breathing, readiness, motivational, or other generic helper copy.
- **That instruction is always rendered, directly under the counter, whatever the repetition count.** It used to be dropped for a zikr said once, which also dropped its height: the counter then sat at a different place on the screen from one zikr to the next, so the control people aim at moved under their thumb between azkar. A counter whose total can only ever be one carries an action on its own face rather than repeating the guidance line beneath it: “Tap when finished” for an ordinary zikr, and for a full surah the mode instruction itself, since the surah is counted only on that control. The line beneath keeps its height there and stays empty, so the surah counter states the instruction once and still sits where every other counter does.
- **A partial tally survives leaving the zikr and coming back.** Counting is per-zikr, but moving to the next or previous zikr does not leave the reader — the screen stays mounted and only the index changes — so an incomplete count is held against the zikr's id for the life of the reading session. Completing or resetting a zikr drops it. Someone who said thirty of a hundred istighfars, glanced at the next zikr and came back keeps their thirty; a reader who leaves the reader entirely starts that zikr again.
- **A multi-page surah reopens on the page it was left.** The place is stored per zikr id in `surahReadingPages` (`src/app/state.ts`), restored when the Mushaf opens, and reset to the surah's first page once the surah is completed, because a finished reading has no place to resume from. Al-Kahf is twelve pages and roughly half an hour of recitation, so being handed page one again after closing the app is that reading repeated.
...
- Quran Wird opens with an opaque overview before the Mushaf: one direction-aware linear progress track with a text equivalent, the last verified reading position, and **Continue reading** as its sole primary action. Its title is **ورد القرآن / Quran Wird** everywhere, and the normal responsive app navigation remains present on this overview. The track fills from logical start, so Arabic fills right-to-left. Pages count only on a forward turn after they were visible; a facing spread records both visible pages as one event, and Undo removes that complete event. Opening or loading a page never completes it. Reading choices are one-at-a-time and always visible as native radio cards: pages per day, finish by the end of the current Hijri month, finish by the end of the current Gregorian month, or free reading. A calendar-month plan names its selected month, uses the days remaining to that real month boundary, and shows one plan-range progress bar; it does not repeat the general Mushaf-position or weekly cards. Free reading saves the current Mushaf position but creates no goals, history, completion state, or progress surfaces. A native selected indicator, semantic border, and visible focus ring make selection perceivable without colour alone. The week is the local Saturday-to-Friday week for a daily-pages plan and shows each day's read/goal count rather than a binary activity mark.
- The standalone 604-page Mushaf **is the whole screen** at every width: no card, no gutter, no letterbox, and no screen padding above or below it. Its permanent header contains Back, the surah/juz index, and Settings; Settings gains a visible text label at the desktop tier. Its permanent footer contains Word meanings, Previous, Page number, Next, and reading place. Header and footer actions share 44px height, compact padding, radius, border, surface, focus, and selected-state treatment, so neither bar contains controls that touch its edges. Both rows are **held to the page's own measure**, so on a 2000px display Previous and Next sit under the paper they turn instead of out at the far corners (DEC-091, DEC-102). The page follows the app's Midnight, Dark, or Light theme by default; an explicit OLED option remains as a high-contrast reading override. Theme treatment uses the same semantic surface, ink, border, accent, and focus tokens as the rest of the app. Desktop and tablet footers show the unobtrusive `← / →` page-navigation hint. **The wird bar reads against the goal chosen on the overview**, computed by the one shared deterministic goal resolver so the two screens cannot disagree. **Physical movement is invariant across interface direction**: an isolated physical LTR group keeps Previous and `ArrowLeft` on the left, and Next and `ArrowRight` on the right; swiping right advances and swiping left goes back in both Arabic and English (DEC-102). A drag moves **the paper only**; button/key turns settle the ready page with a 150ms directional 6px fade using only opacity and transform. Both system and in-app reduced-motion preferences collapse it. The reference is the King Fahd Complex Madani Mushaf in QCF v2, and **all three things that define a page — which words are on it, which line each sits on, and which glyph draws it — come from that one edition** (DEC-090). The 15-line page is a facsimile and its geometry is data, not styling. The standalone reader always preserves that canonical page; the retired reflow reader preference is ignored and removed from persistence. `public/data/mushaf` ships the whole layout, so a page turn makes no page-data network request and the page font is served from Cache Storage after its first load. A page mounts once, already in its final typeface. Every page lays out fifteen equal line slots. **The measure is derived, not chosen**: fifteen lines must fit the page height, which caps the type size, which in turn fixes how wide a line wants to be — so the column is set to exactly that width and the widest line always lands on the margin. Tablet and desktop pages use a slightly lower ink allowance to gain leading without dropping a canonical line or covering the footer. Facing pages are desktop-only, require both the 1024px width and 1.4 aspect-ratio fit gates, and use a 24–48px center gutter; a stored preference can opt out but can never force a spread onto mobile. A line that still overruns is scaled down, never clipped. A surah heading occupies exactly one fixed line slot as a thin semantic-accent curved frame with small mirrored rosettes, real Arabic heading text, and no image, filter, or added height. The basmalah uses the dedicated Mushaf face at one page-container-responsive size that is independent of the page line fitter, so it remains identical at a given viewport whether it owns a full slot or shares a Surah-opening slot. Weight for legibility comes from a hairline text stroke, tuned per theme. An eight-page in-memory cache plus save-data-aware prefetch makes ordinary turns cache hits, and both halves of a facing spread settle concurrently before they replace the visible spread. Word meanings await every visible Surah's complete local chapter file before enabling the study layer; reviewed phrases override matching sourced phrases without hiding the rest of that Surah. Their tint, dotted underline, buttons, and inline metrics cannot move a line. The anchored meaning card reuses the shared word, concise-meaning, ayah, and King Fahd source anatomy, without audio or unsupported root data. Ayah markers expose a keyboard and pointer action sheet for canonical Unicode copy/share and verse bookmarks; QCF PUA glyphs are never copied. The completion notice is opaque, wraps instead of truncating, sits clear of the footer, and dismisses itself after four seconds while retaining its polite status announcement.
...
- **The surah's two bars divide the work.** Where the Mushaf renders bars rather than a rail, the header carries the surah being read and one action — the way out — with the title as the strongest thing in it. Everything else is in the footer: Previous and Next at the outer edges, and between them the word-meanings toggle, the place in the surah, and the recitation. The header names **the reading**, not the page: `surahName` is derived from the first ayah printed on the page, so page 293 — which opens with the tail of Al-Isra — named Al-Isra on a screen opened to read Al-Kahf. What is on the page is named by the page, in its own furniture.
- **Nothing in the chrome repeats what the page prints.** The Mushaf page number and the juz are page furniture — the cartouche and the folio carry them, as a bound Mushaf does — so no bar may print them a second time. The place in the surah (page _n_ of _m_) is not page furniture: it is the reader's position in this reading, it appears once, in the footer, and it doubles as the control that jumps within the surah.
- **Opening pages are text-first.** Pages 1–2 keep their canonical larger line geometry but add no ornamental border or outline. Their visible lines occupy equal grid tracks so spacing remains consistent across phone, tablet and desktop page ratios.
- **Short surahs use the canonical Quran passage without a decorative card.** The Surah header and Bismillah remain, word meanings start off, and enabling meanings is always an explicit reader action.
- **The illuminated opening is drawn, not stretched.** Al-Fatihah and the start of Al-Baqarah carry a panel of concentric rules with a leaf in each corner. The rules are CSS boxes, so a rule keeps its weight at any viewport, and the leaves are the only drawn art — each square, sized in container units against the panel. The panel holds the page`+String.fromCharCode(39)+`s own `+String.fromCharCode(96)+`2 / 3`+String.fromCharCode(96)+` proportions and centres itself in what is left rather than stretching to the viewport, and it is its own size container so the inset, the leaves and the type all measure the panel. Its gilt is `+String.fromCharCode(96)+`--mushaf-rule-ink`+String.fromCharCode(96)+` and its field `+String.fromCharCode(96)+`--mushaf-paper`+String.fromCharCode(96)+`: interface tokens such as `+String.fromCharCode(96)+`--accent`+String.fromCharCode(96)+` and `+String.fromCharCode(96)+`--secondary`+String.fromCharCode(96)+` are not manuscript colours and render as blue or violet in the colour-blind themes. The verse block measures from the same `+String.fromCharCode(96)+`--mushaf-opening-inset`+String.fromCharCode(96)+` the panel does, so type never crosses the gilt.
- **The page is the screen on a phone.** Below the tablet tier the paper ground, its radius and its shadow are dropped and the type is read directly on the app's ground, because there is no shell beside the page to be a sheet against: at 375px they drew a panel of nearly the background colour, with a rounded edge against nothing and a shadow with nothing to cast onto. High contrast already did this for its own reasons. From 640px up the sheet returns, where the page really is a page on a desk. The frame's padding floor drops with it: 20px each side was 11% of a 375px screen spent on margin beside a measure that cannot use it.
- **The reading surface never bounces.** The Mushaf's own scroll containers use `overscroll-behavior: none`, not `contain`: contain stops the gesture reaching the document but still lets the box itself bounce, and on a phone the paper usually has nothing to scroll — so a page-turn drag that drifted a few degrees off horizontal moved the whole reading surface and dropped it back. Nothing above it scrolls, so there is no chaining left to preserve.
...
One floating player serves every recitation (`src/app/components/FloatingAudioPlayer.tsx`); no screen owns playback of its own. It has two forms and both are built from the shared tokens, radii, control heights and focus roles above.

- **Expanded, it reads down the middle**: the playing cue, the title, the reciter, an optional position chip, the timeline, then transport. Stopping sits in the top corner and folding the player away is a wide centred grabber, because collapsing is what someone reaches for mid-recitation and closing is not.
- **Transport is five slots around a circular gold primary**: previous, rewind ten, play/pause, forward ten, next. Each secondary control carries its name under its icon, and that visible label is contained in the control's accessible name (“Rewind” inside “Rewind 10 seconds”), so the two never disagree. A single-track plan drops previous/next rather than filling the slots, keeping the primary centred.
- **The options that exist sit on the surface**, as pills under a rule: replay, repeat when the zikr prescribes a count, playback speed, and the reciter when more than one reviewed recitation exists. Speed shows the rate it is on and steps to the next; the reciter pill reveals the labelled select, which stays a labelled field per the form contract. Nothing here is a control the audio controller cannot actually perform — no volume, no sleep timer.
- **The position chip states only what the audio knows**: the track within a queue, or the prescribed repetition. A single-segment surah has no ayah position, so the chip is absent rather than invented; it becomes an ayah counter only when the recitation itself is segmented.
- **A reviewed audio assignment remains actionable while the audio subsystem is preparing or recovering from a failed load.** The first intentional press may wait for the lazy controller and then start; after a transient chunk failure, the next intentional press retries the controller instead of dimming the action. Module loading itself must never autoplay or imply that an unreviewed recording exists.
- **The timeline fills to the playhead** using the same `--primary` on `--muted` pair as `ProgressBar`, with times at both ends in tabular numerals and an LTR row, since a clock reads Latin-ordered in both interface languages. The separator between reciter and time stays in the surrounding sentence so it is not dragged across the line in Arabic.
- **Compact is one row** — expand, cue, title, reciter and time, play/pause, stop — with the progress hairline on the card's own top edge. **The player opens compact over a reading surface below the tablet tier**: the expanded card takes a large share of a 375px screen, which on the one flow it exists for, listening to a surah while reading along, hides the pages. Opening the Mushaf during playback folds it away too; expanding it again is one tap, and a reader who expands it is left alone.
...
- The five-prayer summary is Home's stable schedule and disclosure layer. Every prayer card contains its own calculated local time; the next prayer card also contains its countdown, with no duplicate device clock. Selecting a prayer keeps the reader on Home and expands that prayer's shared time, virtue, and completion journey immediately below the strip. A rounded notch is anchored to the selected card itself, so it stays aligned as tracks resize and mirror. The prayer buttons expose `aria-expanded` and control the shared panel; its semantic and keyboard order remains unchanged in RTL. The featured routine card stays focused on the selected zikr, its mode, and its progress. Once that selected time-of-day collection is complete, the completion card announces and enters surface/copy/check; it leaves in the reverse check/copy/surface order, then contracts its layout row so adjacent content moves without a jump. The routine CTA remains hidden for that collection until the recommendation changes.
- Each of the five post-prayer cards discloses a prayer-specific reviewed flow on Home. Shared adhkar appear for every prescribed prayer; Fajr and Maghrib additions are admitted only where their cited timing establishes them. Completion is stored under that prayer key and updates both Home and Progress without completing the other four prayers. Home keeps the compact summary anatomy until one prayer is selected, then reuses the focused prayer panel rather than duplicating its tracking rules. The dedicated prayer route and daily Progress view retain the full recording anatomy.
- Home presents its stable five-prayer summary as one responsive strip with five equal tracks at every width. Compact typography and truncation keep all names and times inside that strip without horizontal page scrolling; the current prayer receives the gold focus treatment and a visible Now label. The full tracking anatomy on Progress remains a horizontal snap carousel below the grid tier. Summary and full variants preserve the same five-prayer DOM order and accessible prayer names.
...
- Reduce transparency is also Home's explicit visual-effects preference. When enabled, Home omits the time-of-day photograph rather than leaving it hidden behind an opaque panel, and every top-level Home surface uses the active theme's opaque card tokens. The utility header follows the same theme surface. Glass remains the default when this preference is off and retains its measured on-media contrast and focus treatment.

## Prayer contract

A prayer uses one shared detail surface: Home discloses it directly below the selected prayer summary, while `src/app/screens/PrayerMomentScreen.tsx` remains linkable at `#/prayer/<name>` for focused and Progress entry points. It carries the prayer and its time over its own scene, the narration about praying it in congregation, where it was prayed, its rawātib, and the adhkar that follow — with the day’s five along the foot so the dedicated screen is never a dead end.
...
- **Home gives the prayer moment a bounded turn.** The contextual prayer surface appears during the twenty-minute approach and for thirty minutes after the adhan. Recording congregation keeps it visible for a five-minute confirmation grace, then Home yields the contextual space to the current zikr routine. The stable five-prayer summary remains available throughout; the dedicated prayer screen remains available from every summary card.
...
- **Congregation is the tracked distinction.** The first journey step records whether the prayer was completed in congregation. Legacy `location: "home"` records still normalize safely, but current UI does not ask the reader to classify a prayer at home as a lesser branch.
...
- **A sunnah names its rank and its narration.** The confirmed rawātib and the encouraged rak‘ahs outside them are different rulings, so the card says which it is — `صلاة الرواتب` or `سنة مستحبة` — rather than leaving the reader to infer it from the layout. Every entry carries the narration it rests on, with its collection, number and, where it sits outside Bukhari and Muslim, its grading; an information control opens it above the card’s own input, so reading why never records a prayer.
- **The update prompt applies the update.** The app knows the release it was built from, so a deployed release that differs is offered as an update with an action that applies it, rather than as a recap of features the reader does not have. The service worker still waits for the reader rather than taking over mid-session.
- **Each prayer has its own drawn sky on the focused prayer screen**, in `PrayerSceneArt`: vector rather than photography, so there is no request to wait for, the hue holds in every theme, and the precache gains nothing. It is composed rather than drawn as one picture, because the hero has no fixed aspect ratio — the sky is a CSS gradient, the light is a fixed-size element placed at a percentage, and only the skyline is SVG, **fitted** to the card's width and anchored to its bottom edge. On Home, the contextual prayer uses one `hero-glass` surface over the existing time-of-day photograph; its hero, virtue, journey, and final action are sections divided by hairlines, never nested glass cards. Text over either scene uses fixed light-on-dark tokens.
- **The page keeps to the shared measure, and its rows to their content.** The prayer screen uses `.page-content-center` with `auto-rows-min` and `content-start` from the tablet tier. Without the measure it stretched across a 2000px display; without content-sized rows the grid spread three bands down the viewport with empty card between them.
- **The prayer card on Home is a disclosure, and says so.** Its native button names the prayer, exposes expanded state, and controls the shared panel directly below the strip. The selected card owns the responsive notch, avoiding direction-specific coordinate arithmetic. In the full Progress variant, the first tracking row reads **Prayed**, ticks for either recorded place, and names that place beneath it: rendering `mosque` alone meant a prayer recorded at home showed here as an empty box, which reads as unrecorded.
...
- Home keeps the five-prayer summary above its contextual hero row at every tier. From the 1024px component tier onward, the primary and daily-evidence cards use two equal fluid tracks: each receives `(available width - gap) / 2`, so neither hard-coded widths nor content can create a clipped third column. The five prayer summaries use five `minmax(0, 1fr)` tracks at that tier and a snap carousel below it; desktop never keeps a horizontal prayer scrollbar. Both contextual cards stretch to the row height. The prayer hero contains the approved interactive post-prayer journey; the summary above it is stable prayer navigation and temporal status, not a second set of tracking controls. Today's Wird occupies its own full-width row so its three routine tiles respond to real container space instead of being compressed inside a companion column. The complete Home composition stays inside the 90rem dashboard measure and stacks in semantic order below the desktop tier.
- The time-of-day photograph is absolutely contained by Home's main-content surface; it must never be fixed to the browser viewport, because that lets the later-painted main layer cover the expanded rail or large sidebar. Every top-level Home card over that photograph opts into `hero-glass`; components that normally use theme foreground/card tokens add `home-glass-surface` so text, borders, and nested controls use the measured on-media palette. Reduced transparency turns every such surface opaque through the shared fallback.
