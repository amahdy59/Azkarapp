...
- Quran Wird opens with an opaque overview before the Mushaf: one direction-aware linear progress track with a text equivalent, the last verified reading position, and **Continue reading** as its sole primary action. The track fills from logical start, so Arabic fills right-to-left. A page is recorded only by the explicit action on that page in the reader, never from the overview and never merely by loading it. Plans are one-at-a-time (30-day khatmah, daily pages, or custom duration), sit behind native progressive disclosure, and use gentle redistribution language rather than streak failure. The week is the local Saturday-to-Friday week, matching the app's Arabic-first context rather than a rolling seven-day strip.
- The standalone 604-page Mushaf keeps exactly one visible page indicator in the fixed navigation row. It does not show a Khatmah percentage or Wird progress; those belong to the Wird overview. The page surface uses the reviewed 15-line word/line data and the offline Amiri Quran font to preserve a recognisable Madani rhythm, with a flat full-bleed phone treatment and one bounded page surface on wider screens. This is a semantic, source-aware Mushaf presentation, not a claim of pixel-identical QCF typesetting: exact print facsimile requires the Quran Foundation per-page QCF glyph codes and page fonts. Reviewed difficult words are always visible as buttons with tint plus dotted underline, not hidden behind a mode toggle. Page turning supports horizontal drag/swipe, physical arrow keys, and persistent single-pointer Previous/Next buttons; reduced-motion removes the lateral transition without changing the destination.
