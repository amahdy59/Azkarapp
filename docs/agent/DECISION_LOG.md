...
- **Decision:**
  - **Where the screen is landscape and at least 900px wide, the tools stand in a vertical rail beside the paper and the two horizontal chrome bars are not rendered at all.** The rail is 72px on desktop and 60px below 1200px, carries every control the bars carried under the same accessible names, and keeps the page-turn pair inside a named `navigation` landmark. Portrait screens keep the bars, because there it is width that is short.
