...
- **Decision:** Replace the plan-type dropdown with four native radio-card choices: pages per day, finish within the current Hijri month, finish within the current Gregorian month, and read freely. Month plans use the actual length of the selected current calendar month and calculate the adaptive daily target from the reader's current page. Free reading continues to persist the current Mushaf position but does not record Wird history, daily goals, completion, or progress summaries. Existing legacy 30-day/custom plans remain readable and map to the Hijri-month choice when edited.
