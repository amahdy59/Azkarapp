...
- **2026-09-11 dependency-owned storage addendum:** prefix sweeping only stays complete for namespaces the application knows about. Supabase writes `sb-*` session tokens and the audio-review tool writes `azkar.audio-content-review.*`; both now belong to full erasure. Full erasure attempts remote sign-out but still removes the local token if offline. Ordinary account sign-out uses a narrower private-cache sweep so search/location remnants go while device preferences and downloads stay.
- **Supersedes:** None
...
## DEC-098 — one theme vocabulary and measured Mushaf comfort

- **Date:** 2026-08-24
- **Status:** Approved
- **Owner:** User (approved the recommended treatment for the remaining seven items)
- **Related scope:** Category list, Palm disclosure, Home completion, standalone Mushaf, word meanings, theme persistence, tests
- **Decision:** Number badges use an opaque primary surface and contrasting numeral. The Palm disclosure stays in flow with shorter professional bilingual copy. Home completion enters surface, copy, then check and exits in reverse before its row contracts. Facing-page content aligns toward the spine with a maximum 64 px gutter; tablet and desktop reduce ink allowance slightly to add leading without removing any of the fifteen page lines or covering the footer. The standalone Mushaf follows the app's Midnight, Dark, or Light theme by default, with OLED retained only as an explicit high-contrast override; legacy parchment and white preferences migrate to follow-app and Light respectively. Difficult-word help reuses the shared anchored popover and adds the concise-meaning label, ayah, and authentic King Fahd source, without audio or unsupported root derivation.
- **Why:** These changes solve the reported contrast, clipping, dead-space, reading-comfort, and visual-consistency defects at shared boundaries. Static Arabic DOM and compositor-friendly transforms preserve reading order, reduced motion, and offline page layout.
- **Tests/evidence required:** preference migration, app-theme resolution, semantic dark chrome, sourced popover anatomy, capped spread hooks, symmetric completion hooks, focused mobile/desktop browser screenshots, and the full release gates.

## DEC-099 — Quran progress is event-based and plan ranges are explicit

- **Date:** 2026-08-24
- **Status:** Approved
- **Owner:** User (requested completion and correction of the consolidated audit plan)
- **Related scope:** Quran progress persistence, Home resume card, Wird overview, Mushaf reader, state merge and normalization
- **Decision:** One devotional-day key is threaded through Quran and azkar progress; the product boundary remains the fixed local-midnight rule in DEC-083, while components continue to accept the configured boundary input for compatibility and seam testing. A forward page turn records the pages that were actually visible, including both pages of a spread, as one undoable reading event. Loading a page never completes it. Current location, the one intentional reading bookmark, completed pages, whole-page bookmarks, and verse bookmarks remain separate state. Adaptive plans persist an inclusive start page and target page, calculate against a supplied day key rather than the system clock, snapshot each day's displayed goal, and expose expiry as an action state rather than completed progress.
- **Consequences:** Home can distinguish a first reading from a real continuation; the overview shows exact page, surah, juz, daily progress, Khatmah orientation, quantitative Saturday-to-Friday history, and whole-event Undo. State normalization rejects malformed settings and merges same-day pages without losing concurrent progress.
- **Tests/evidence required:** 03:00-boundary seam coverage at 01:30, inclusive range arithmetic, explicit expiry, spread-aware recording and Undo, persisted/merged state coverage, and the full release gates.

## DEC-100 — canonical Mushaf geometry and study actions have separate interaction layers

- **Date:** 2026-08-24
- **Status:** Approved
- **Owner:** User (requested the remaining reader accessibility and ayah-study work without empty placeholders)
- **Related scope:** Mushaf page viewer, Comfort mode, navigation index, ayah actions, directionality, accessibility
- **Decision:** The 15-line page is a facsimile and its geometry is data, not styling. Page mode may adjust the ink-to-slot allowance but must not reflow or alter the fifteen-line geometry. Larger text belongs in a separate Comfort mode that reflows canonical Unicode Quran text. In QCF mode, visible glyphs are PUA code points and semantic Uthmani text is carried separately, so selection-copy is broken by construction; tap or long-press opens an ayah sheet whose copy/share actions reconstruct canonical reviewed text across page boundaries. Ayah bookmarks persist verse key plus authoritative page and navigate back with temporary highlighting. Translation, Tafsir, and Quran-wide search remain deferred until reviewed local data and an offline index exist; no coming-soon controls are shown.
- **Direction rule:** The article remains RTL because Quran reading order is RTL. Application chrome inherits the current UI direction. The page-turn group alone uses an isolated physical LTR layout: forward/ArrowLeft stays on the physical left and back/ArrowRight on the physical right in both Arabic and English. DOM and keyboard order stay stable and are never swapped by language.
- **Accessibility:** Index navigation uses the APG tab pattern; each visible Mushaf page has its own named structure; page, surah, and juz changes are announced; page keys, Home, End, and Escape are scoped away from interactive controls and modal surfaces; ordinary actions retain 44 px targets and visible 3 px focus.
- **Tests/evidence required:** Arabic and English direction-seam tests, tab semantics and keyboard behavior, canonical cross-page copy, ayah bookmark navigation, Comfort-mode controls, page-level structure, focused browser screenshots, and the full release gates.

## DEC-101 — optional navigation motion must never weaken navigation or dialog semantics

- **Date:** 2026-08-24
- **Status:** Approved
- **Owner:** User (requested a full post-deployment scan and implementation of verified improvements)
- **Related scope:** shared View Transitions utility, responsive dialog/sheet primitive, reduced-motion and accessibility contracts
- **Context:** Production navigation completed successfully but an optional View Transition could reject its `ready` promise when a newer transition won or the browser timed out the animation. Because the promise was ignored, the expected animation skip leaked as an unhandled console error. The same scan found that every shared modal and drawer repeated its accessible title as a hidden description, making assistive technology announce the title twice. The motion helper also assumed `matchMedia` existed even though the application explicitly supports embedded browsers where it may not.
- **Decision:** Treat the View Transitions API strictly as progressive enhancement: guard `matchMedia`, keep the destination update synchronous, and consume only the animation-level `ready` rejection. Do not consume `updateCallbackDone` or `finished`, so callback failures remain observable. Shared dialogs and sheets expose one hidden title and use only a caller-supplied `aria-describedby` relationship when meaningful description copy exists; they never invent a description by repeating the title.
- **Consequences:** A browser may skip decorative navigation motion without producing an unhandled rejection or blocking the destination. Reduced-motion and older embedded-browser paths remain safe. Dialog announcements become concise while existing Ayah, word-meaning, and reference descriptions remain connected.
- **Tests/evidence required:** unit coverage for reduced motion, missing `matchMedia`, rejected transition readiness, accessible dialog names, and caller-owned descriptions; focused Quran/accessibility/PWA/offline browser coverage; full release gates and production verification.

## DEC-102 — the canonical Mushaf stays visible, contextual, and physically predictable

- **Date:** 2026-08-24
- **Status:** Approved
- **Owner:** User (reference screenshots plus explicit requests for compact ornament, smaller basmalah, desktop-only facing pages, permanent controls, right-forward gestures, removal of the old reader, a quieter settings menu, and faster spread turns)
- **Related scope:** `KhatmahReaderScreen.tsx`, `MushafPageViewer.tsx`, `MushafImmersiveReader.tsx`, Mushaf preferences, responsive layout, tests
- **Decision:** The standalone reader always renders the canonical 15-line facsimile. The reflow/Comfort preference and keep-controls-visible preference are retired and dropped during normalization. Header and footer controls stay visible. The settings menu exposes theme and, only where two pages physically fit, desktop layout through nested submenus; page bookmark remains one action row. Facing pages can never be forced below the 1024px and 1.4 aspect-ratio fit gates. Their center gutter is 24–48px. Swipe right, `ArrowRight`, and the right-hand Next button advance; swipe left, `ArrowLeft`, and the left-hand Previous button go back in Arabic and English. Both halves of a desktop spread load concurrently and replace the prior spread together. Surah openings use a compact symmetric in-code arabesque and a smaller basmalah in the dedicated Mushaf face.
- **Why:** The canonical page is the product; keeping an alternate numbered reflow view in the same menu made navigation appear to return to an obsolete reader and encouraged geometry changes the facsimile contract forbids. Permanent orientation and semantic direction reduce relearning. Nested, contextual settings remove choices that cannot affect the current viewport. Concurrent spread settlement removes the visible single-page intermediate state that made local desktop turns feel sluggish.
- **Consequences:** DEC-091's auto-hide behavior, DEC-094/DEC-100's paper-bound direction mapping, and DEC-100's standalone Comfort mode are superseded. Existing stored values are safely ignored rather than migrated into new behavior. The long-surah immersive reader keeps its separate fit/comfort presentation but adopts the same physical key and button direction.
- **Tests/evidence required:** ornament and basmalah typography checks, permanent chrome after the old idle window, mobile forced-spread refusal, desktop-only layout menu, concurrent facing-page load, Arabic/English button-key-swipe parity, focused mobile/desktop screenshots, full local gates, and production verification.

## DEC-103 — Surah openings use a fixed curved frame and fitter-independent Bismillah

- **Date:** 2026-08-24
- **Status:** Approved
- **Owner:** User (option 1 or 3 reference, explicit request for consistent Bismillah sizing, in-code ornament, real Arabic text, and unchanged fifteen-line geometry)
- **Context:** DEC-102 made the Surah heading compact, but the first implementation reduced the reference to a plain rule-and-flourish band. Its `em`-based Bismillah also inherited each page's fitted Quran size, so two Surah openings at the same viewport could render the same phrase at different sizes. Nineteen Surahs still require the heading and Bismillah to share one canonical line slot, so adding arbitrary padding or height would corrupt page geometry.
- **Decision:** Use the reference's modern-curved option: one reusable, non-interactive inline SVG outline with two small mirrored rosettes, semantic accent colour, and a real Arabic `h2`. It receives the existing line slot and cannot create another one. Both ordinary and combined openings render their existing reviewed Bismillah strings through one component whose pixel-and-container clamp is independent of the per-page line fitter. A combined opening divides its one canonical slot internally between the compact heading and the same-size Bismillah. At-Tawbah remains the no-Bismillah exception. No image asset, filter, dependency, Quran text, line inference, slot count, or page-fit constant changes.
- **Consequences:** This supersedes DEC-102's visual wording for the Surah ornament while preserving its compactness and all canonical pagination contracts. At the same viewport, every Bismillah has the same computed size; only the heading tightens inside a shared slot. Theme differences come entirely from semantic tokens.
- **Tests/evidence required:** unit checks for shared typography, semantic heading and decorative SVG; a 320×568 browser regression across ordinary, combined, and At-Tawbah openings; fifteen-slot, clipping and overflow assertions; long-name, all-theme and desktop-spread visual review; full release gates and production verification.

## DEC-104 — reading choices are visible, calendar-aware, and optional

- **Date:** 2026-08-26
- **Status:** Approved
- **Owner:** User (explicit request for simpler Hijri/Gregorian choices, free reading, RTL repair, and accessible selection colour)
- **Related scope:** Quran Wird overview, Mushaf progress recording, Home Quran card, persistence, i18n, tests
- **Decision:** Replace the plan-type dropdown with four native radio-card choices: pages per day, finish by the end of the current Hijri month, finish by the end of the current Gregorian month, and read freely. Month plans use the inclusive days remaining to the selected calendar's real month boundary and calculate the adaptive daily target from the reader's current page. Free reading continues to persist the current Mushaf position but does not record Wird history, daily goals, completion, or progress summaries. Existing legacy 30-day/custom plans remain readable and map to the Hijri-month choice when edited.
- **Accessibility:** The fieldset and native radio inputs provide a named single-choice group, keyboard operation, and a visible selected indicator. Arabic content inherits RTL layout and also declares right text alignment. Selected cards use semantic foreground/background/border tokens and do not rely on colour alone.
- **Consequences:** The Quran Wird plan editor no longer uses the shared Select primitive; this task-specific exception supersedes the corresponding Quran sentence in the earlier menu-unification guidance. No dependency, Quran text, pagination, or Mushaf geometry changes.
- **Tests/evidence required:** normalization and calendar arithmetic tests; free-mode no-record coverage; RTL radio alignment; focused mobile/tablet/desktop browser coverage; full release gates and production verification.

## DEC-105 — Mushaf chrome is one control system and month plans report one outcome

- **Date:** 2026-08-26
- **Status:** Approved
- **Owner:** User (screenshots and explicit request for cohesive bars, labelled desktop Settings, repaired Word meanings, a performant page transition, and clearer month progress)
- **Related scope:** standalone Mushaf chrome, complete chapter meanings, page-turn motion, Quran Wird overview, i18n, tests
- **Decision:** Top and bottom Mushaf actions share a compact 44px control height, padding, radius, semantic borders/surfaces, and focus treatment inside the existing 56px bars. Settings remains icon-only on compact/tablet widths and adds its text label on desktop. Word meanings waits for all visible Surah chapter files before switching on; the complete sourced chapter is merged beneath any reviewed bundled phrases instead of being hidden by a partial reviewed map. A settled button/key page turn uses a 150ms, 6px directional entrance limited to opacity and transform; drag behavior stays direct and reduced motion collapses the animation. Hijri/Gregorian plans name their selected month and show one page-range progress bar; the duplicate Mushaf-position card and weekly card are omitted for month plans.
...
## DEC-108 — reader references show the narration and its citation without editorial duplication

- **Date:** 2026-08-27
- **Status:** Approved
- **Owner:** User (explicit request to remove duplicate handles, merge benefit and evidence, keep hadith and sources, and remove Arabic-view English leakage)
- **Related scope:** Reader reference trigger, responsive reference sheet/dialog, localized source rendering, accessibility and responsive tests
- **Decision:** Rename the reader action from Benefit to Reference because the surface now contains evidence rather than editorial benefit copy. On compact layouts, retain Vaul's single swipe handle and remove the component's duplicate handle. Remove the repeated zikr identity, editorial benefit, preferred timing, Surah translation, and separate authenticity note from this surface. Render only the Arabic hadith, its one copy action, and the language-localized source citation. Let the drawer size to its content up to the existing safe viewport cap instead of forcing an 82dvh height.
- **Preserved:** Reviewed zikr, hadith, source and authenticity data remain unchanged; source labels including Bukhari, Muslim and other collections remain available through the existing localized citation boundary. The shared dialog/drawer focus trap, Escape dismissal, focus restoration, semantic headings, 44px targets and theme tokens remain intact.
- **Tests/evidence required:** single-handle mobile visual review, compact desktop review, Arabic no-English assertion, two-section hierarchy, copy/close behavior, short-screen fit, automated accessibility scan, full local gates, and production verification.

## DEC-109 — landscape Mushaf tools stand beside the paper, and each page names itself

- **Date:** 2026-09-01
- **Status:** Approved
- **Owner:** User (annotated desktop/tablet/mobile UI proposal: a fixed vertical toolbar, a focus mode, a phone overflow menu, a toolbar-side choice, and a reading type size)
- **Related scope:** `MushafToolRail.tsx` (new), `MushafQuickMenu.tsx` (new), `MushafPageViewer.tsx`, `MushafSettingsSheet.tsx`, `MushafNavigationModal.tsx`, `KhatmahReaderScreen.tsx`, `surahInfo.ts`, Mushaf preferences, `layout.css`, i18n, unit and Playwright tests
- **Decision:**
  - **Where the screen is landscape, at least 768px wide, and at least 520px tall, the tools stand in a vertical rail beside the paper and the two horizontal chrome bars are not rendered at all.** The rail is 72px on desktop and 60px below 1200px, carries every control the bars carried under the same accessible names, and keeps the page-turn pair inside a named `navigation` landmark. Portrait screens keep the bars, because there it is width that is short; so does a phone held sideways, which would need the rail most and can hold it least — a toolbar you have to scroll to reach is worse than one that fits.
  - **The reader chooses which edge the rail is pinned to** (`mushafToolbarSide`, default right). The choice is offered only where a rail is actually shown, and the rail's back arrow points away from the reading surface accordingly.
  - **Focus mode is an action, not a stored preference.** It hides the rail or the bars entirely, leaves a hairline handle, and is undone by the handle, by `Escape`, or by a tap on the paper that is not a swipe and not on a word or ayah marker. `Escape` gives the tools back before it gives up the reader. Nothing hides on a timer: DEC-090 removed an automatic hide window and this does not reintroduce one.
  - **A phone's secondary actions live one tap behind an overflow button** — index, saved bookmarks, page bookmark, reading place, word meanings, focus mode, and settings — instead of competing for the header. The Settings control keeps its place in the bar from 640px up.
  - **Reading type size is a three-step choice** (`mushafTextScale`, default medium) that scales the ink allowance inside the fifteen slots by 0.9/1.0/1.08 and is clamped at 0.94. It cannot add, remove, or re-break a line: pagination and line breaks are page data.
  - **Each page carries its own surah cartouche, juz, and folio number** in bands outside the fifteen slots — but only where that is not a second copy of what the chrome already says. A spread, a rail layout, and focus mode print the identity; a phone or portrait tablet with a header naming the same surah does not. Every page gains a hairline manuscript frame in either case, drawn at the width of the page's own measure rather than the full canvas, so it reads as paper rather than as a box around a column.
  - **Each half of a spread owns its own measure.** The line fitter wrote `--mushaf-measure` and `--mushaf-fit` to the shared parent of both canvases, so whichever page settled last sized both of them; it now writes to its own canvas.
  - **The reading settings do not hide the page they change.** Theme, type size, and layout all act on what is behind the sheet, so this one surface drops the scrim's backdrop filter; every other modal keeps it. Where a rail is showing there is width to spare, so the settings become a full-height panel docked to the rail's own edge and held back by the rail's width, leaving the rail visible and a whole page of the spread uncovered. It is a Radix modal like every other dialog — focus containment, `Escape`, and focus restore are unchanged (DEC-025) — with a lighter scrim. Elsewhere it stays a drawer on compact and a centred sheet on medium.
  - **Choosing a theme leaves the settings open.** It used to close them, which made the one control whose entire point is comparison the only one you could not try twice, and which — once the settings were docked beside the page — also took away the view of what the choice had just done.
  - **The raw CSS budget rises from 160 kB to 164 kB**, on an explicit user decision, to fund the docked panel (422 bytes). The 160 kB ceiling had drifted to roughly 1 kB of headroom: it had stopped separating "this feature costs a little CSS" from "the purge broke again" (DEC-064, a 15.7 kB regression), and was failing the first while existing to catch the second. The gzip ceiling is unchanged, because that is the number that reflects what ships over the wire, and it still holds 1.4 kB.
- **Why:** On a landscape screen height is the scarce dimension and width is not, so 112px of horizontal chrome was the most expensive furniture in the reader. Standing it on end returns that height to the page without hiding anything. The per-page identity is what a bound Mushaf does and what makes a spread legible — the chrome can only ever name one of two pages.
- **Adopted after review:** the proposal's docked settings panel, which was initially deferred for bundle-budget reasons and then funded by the raise above.
...
## DEC-130 — the attributions speak Arabic

- **Decision:** `attributionArabic.ts` holds Arabic renderings of the gradings,
  benefits and source references the content carries in English, resolved by the
  reader's language with the English as fallback.
- **What was wrong:** the app is Arabic-first, but all 146 authenticity notes,
  122 of the benefits and 142 of the source references existed only in English.
  The daily narration card made it visible: Arabic hadith under an English
  citation, on the one surface whose whole purpose is to present evidence.
- **These are translations, not new claims.** No grading is added, softened or
  strengthened. The English stays authoritative, and a wrong rendering can be
  corrected here without touching reviewed content.
- **Keyed by the English string, not by zikr id,** because one attribution
  serves many azkar — 37 distinct gradings cover all 146 — so one wording lives
  in one place. The file was generated by zipping two lists so that no English
  key was ever retyped by hand.
- **Source references are transformed, not translated.** A citation is a
  locator: collection names map to canonical Arabic (with the genitive the
  construct needs — `Sunan Abu Dawud` is سنن أبي داود, not أبو) and the numbers
  are converted to Arabic-Indic digits programmatically. No number was retyped,
  which is the one error that would send a reader to the wrong hadith. A
  trailing letter on a number is Sunnah.com's own sub-reference and stays.
- **Tests/evidence required:** every grading, benefit and source reference the
  content uses has a rendering; each rendering is in Arabic script; no English
...
## DEC-167 — closed-app push begins with a reproducible account security baseline

- **Decision:** Phase 25 is split. Phase 25A first makes the existing optional Supabase account boundary reproducible and least-privileged; subscriptions, VAPID material, scheduled delivery and production enablement belong to later Phase 25 slices.
- **Why this is upstream:** the prior audit proved that migration replay did not create the four base account tables or their RLS policies. Adding push subscriptions to that history would make a fresh project unsafe even if the new table were correct.
- **Migration authority:** ordered files under `supabase/migrations/` are the only deployment source of truth. `schema.sql` is a generated current-state review snapshot, never a second path to apply alongside migrations. A fresh replay creates the base tables, enables RLS, adds owner policies, and revokes implicit Data API privileges before granting only the operations the browser uses. A final hardening migration applies the same grants to existing projects.
...
## DEC-177 — Wide navigation expands utilities and listening owns its progress

- **Responsive navigation:** supersede DEC-175 only at the large labelled-sidebar tier. Bottom and rail navigation retain Home, Azkar, Progress, and More; the large sidebar omits More and directly exposes Qibla, Masbaha, and Settings.
- **Reader language:** supersede DEC-176's reader-action naming and icon split. The action is Benefit, uses the Lightbulb, and keeps its text visible on phones as well as wider layouts. Its sheet presents the reviewed benefit first, followed by the supporting narration and source. Overflow actions are grouped by task, with text-size controls nested under Display.
- **Home composition:** an expanded half-width prayer card is always paired with the current routine reminder, then daily evidence where available. A later dashboard card is not pulled out of semantic order merely to fill the track.
...
## DEC-186 — Mushaf pages are borderless and desktop spreads are an explicit responsive choice

- **Decision:** supersede DEC-180 and DEC-185 only where they prohibit facing pages or expose every desktop tool at once. Preserve one canonical page on compact and portrait surfaces. On qualifying landscape surfaces, restore the existing Automatic, Single Page, and Two Pages preference; explicit Two Pages still cannot bypass the physical-fit gate, and Automatic chooses a spread only when each page retains a comfortable measure.
- **Surface:** no Mushaf page becomes a bordered, rounded, shadowed card or paints a second background at any width. The application-level Surah cartouche is borderless; the authentic curved Surah banner inside the canonical page remains.
- **Navigation:** the standalone desktop rail keeps Back, index, page movement, bookmark, and word meanings visible, then exposes secondary actions through one More control. Mobile retains the four corner actions. Word meanings uses the same Translate glyph in both variants.
...
## DEC-205 — Finite Masbaha intentions and truthful Progress comparisons

- **Date:** 2026-09-24
- **Status:** Approved
- **Owner:** Product owner (explicit implementation request)
- **Related phase:** Phase 64
- **Context:** The dedicated Masbaha allowed an open-ended target, exposed reviewed remembrances through an unsearchable compact selector, and could discard a non-empty session when changing remembrance or resetting. Progress mixed a configurable weekly intention into fixed seven-day activity summaries, omitted the final days of long months from annual heatmaps, and retained stale or ambiguous visual labels.
- **Decision:** The dedicated Masbaha always resolves to a positive finite target, defaulting legacy or new open sessions to the selected remembrance's reviewed recommendation. It offers finite presets and a validated custom value, preserves the current count when only the target changes, confirms destructive reset or remembrance replacement, and keeps a completed counter actionable so its completion options can be reopened. The remembrance picker is a searchable labelled dialog that shows the exact reviewed phrase and concise benefit with the shared `Lightbulb` icon. Progress keeps routine frequency denominators fixed at seven days, displays the weekly intention as a separate labelled measure, renders every calendar day, and uses neutral unrecorded states without a stale partial legend.
- **Why:** A counter is more useful when it represents a clear intention, while selection and reset actions must protect effort already recorded. Progress comparisons must describe observed data honestly rather than changing their scale according to a preference.
- **Consequences:** Existing persisted open Masbaha sessions normalize safely without a storage migration; target changes no longer erase counts; selecting a different remembrance deliberately begins a fresh session after confirmation. Weekly activity remains comparable while the user's intention is still visible. No religious source text or repetition evidence is changed.
