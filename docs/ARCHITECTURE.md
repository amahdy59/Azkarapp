...
- Qibla calculation is local and deterministic from coordinates. Live compass rotation is progressive enhancement: it is requested from a user action, requests absolute orientation permission when the browser supports the standards-based argument and retries Safari's earlier no-argument method only for a compatibility TypeError. A denied permission or other rejection is never bypassed. The dial accepts only an absolute-orientation event or a nonnegative WebKit compass heading, rejects relative gyroscope headings, rotates both the north marks and Qibla arrow from valid headings, smooths across the north seam without delaying deliberate large turns, and leaves a clearly labelled static bearing usable when sensor permission or hardware is unavailable. Fine-pointer large screens present the same bearing as a static prayer-space alignment method instead of offering a sensor control that is normally unavailable; this is presentation-only and introduces no second bearing calculation or persisted state.
