  ...
  return (
    <div className="slide-in-from-right flex h-full flex-col bg-background/50 backdrop-blur-md">
      ...
      <div className="flex-1 space-y-4 overflow-y-auto p-5">
          ...
          <>
            ...
            >
              <div className="flex items-baseline justify-between gap-3">
                <h2 id="weekly-goal-title" className="text-subtitle font-bold text-foreground">
                ...
                <div className="grid grid-cols-4 gap-2">
                  {[3, 4, 5, 7].map((goal) => (
                    <RadioGroupPrimitive.Item
                      key={goal}
                      value={String(goal)}
                      className={`min-h-11 rounded-xl border px-2 text-label font-bold focus-visible:outline-none focus-visible:ring-[3px] focus-visible:ring-ring ${
                        weeklyGoalDays === goal
        ...
        >
          <h2 id="progress-day-boundary-title" className="text-subtitle font-bold text-foreground">
            {t(language, "progressPanel.dayBoundaryTitle")}
          </h2>
          <p className="mt-1 text-xs leading-5 text-muted-foreground">{t(language, "progressPanel.dayBoundaryHint")}</p>
          <div className="mt-3 flex min-h-11 items-center justify-between gap-3 text-label font-semibold text-foreground">
        ...
        <section aria-labelledby="recent-sessions-title">
          ...
          <div className="space-y-2">
            {completedSessions.slice(0, 5).map((session) => (
              ...
              >
                <div className="min-w-0 flex-1 text-start">
                  <p className="text-sm font-semibold text-foreground">{categoryName(session.category, language)}</p>
            ...
            {completedSessions.length === 0 && (
              <p className="rounded-2xl border border-dashed border-border p-6 text-center text-label text-muted-foreground">
