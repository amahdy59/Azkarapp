...
export function ArabicWelcomeScreen({ onNext, onSkip }: { onNext: () => void; onSkip: () => void }) {
  return (
    <div className="flex h-full flex-col bg-background slide-in-from-right" dir="rtl">
      ...
      <div className="flex flex-1 flex-col items-center gap-7 px-6 pb-7 pt-5" style={{ fontFamily: "'IBM Plex Sans Arabic', sans-serif" }}>
        ...
        <button onClick={onNext} className="h-[52px] w-full rounded-2xl border-2 border-white/10 bg-primary text-[16px] font-bold text-primary-foreground shadow-[inset_0_-2px_0_rgba(10,13,18,0.12)] transition-transform active:scale-[0.98]">
          ابدأ الآن
        </button>
        <button onClick={onSkip} className="sr-only">تخطي</button>
      </div>
    </div>
  );
}
