    ...
    <div className="flex flex-col h-full bg-background slide-in-from-right">
      <div className="flex flex-col flex-1 px-7 pt-6 pb-6 gap-7">
        ...
        <button onClick={phoneAuthEnabled ? onPhone : undefined}
          ...
          <div className="flex items-center justify-center rounded-[18px] shrink-0 w-[36px] h-[36px] bg-[#14B8A6]">
    ...
    <div className="flex flex-col h-full justify-between bg-background slide-in-from-right">
      <div className="flex flex-col">
        <div className="flex items-center justify-between px-5 shrink-0 h-[56px]">
          <button onClick={onBack} className="flex items-center justify-center w-[44px] h-[44px] active:scale-95 transition-all">
            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" className="text-foreground rtl:-scale-x-100">
              <path d="M15 18L9 12L15 6" stroke="currentColor" strokeWidth="2" strokeLinecap="round" />
            </svg>
          </button>
          <p className="text-[17px] font-semibold text-foreground font-sans">
            {t(language, "auth.signIn")}
          </p>
          <button onClick={onSkip} className="active:scale-95 transition-all">
            <p className="text-[17px] font-semibold text-muted-foreground font-sans">
        ...
        <div className="px-6 pt-6 flex flex-col gap-8">
          ...
          <div className="flex items-center rounded-xl gap-3 px-3 h-[60px] bg-card border-[1.5px] border-primary focus-within:ring-2 focus-within:ring-ring focus-within:ring-offset-2">
            <div className="flex items-center gap-1.5 shrink-0 rounded-lg px-2 py-1 border border-[#14B8A6]">
              <span className="text-[14px] font-sans text-foreground">🇸🇦 +966</span>
          ...
          <div className="flex items-center gap-2">
            ...
            <p className="text-[11px] text-muted-foreground font-sans">+190 countries supported</p>
          </div>
          {errorMessage && (
            <p className="text-[12px] text-destructive font-sans leading-[18px]">
              {errorMessage}
            </p>
          )}
        </div>
        ...
        <div className="px-6 pt-4 flex flex-col gap-8">
          <div className="flex flex-col gap-2">
            <p className="text-[13px] text-muted-foreground font-sans leading-[20px]">
              ...
              <span className="text-foreground" dir="ltr">{maskedPhone}</span>
            </p>
          ...
          <div className="flex gap-2 justify-between" dir="ltr">
            {digits.map((d, i) => {
              ...
              return (
                <div key={i} className={`relative flex items-center justify-center rounded-lg w-[52px] h-[60px] shrink-0 border-2 ${isEmpty ? 'bg-muted border-transparent' : 'bg-card'} ${isActive ? 'border-primary' : 'border-transparent'}`}>
                  {isFilled
                    ? <div className="rounded-full w-[8px] h-[8px] bg-foreground" />
                    : isActive
                    ? <div className="w-[2px] h-[24px] bg-primary rounded-[1px] animate-pulse" />
                    : null
                  }
                  <input
          ...
          <p className="text-[13px] text-muted-foreground font-sans leading-[20px] text-center">
            ...
            {countdown === 0
              ? <span className={`text-primary font-semibold ${isResending ? 'cursor-wait' : 'cursor-pointer'}`} onClick={() => { if (!isResending) { setCountdown(60); onResend(); } }}>{isResending ? t(language, "common.resending") : t(language, "common.resend")}</span>
              : <span>{t(language, "auth.resendIn", { seconds: countdown })}</span>
            }
          </p>
          {errorMessage && (
            <p className="text-[12px] text-destructive font-sans leading-[18px] text-center">
              {errorMessage}
            </p>
          )}
        </div>
